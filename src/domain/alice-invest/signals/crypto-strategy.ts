...
export function evaluateCryptoSpotStrategy(input:CryptoStrategyInput):SignalCandidate|null {
  ...
  const all=[...input.fast,...input.slow], latest=input.fast.at(-1)!; if(all.some(x=>x.symbol!==input.symbol||x.capability!=='realtime')||!assessObservationFreshness(latest as NormalizedMarketObservation,input.maxAgeSeconds,input.now).fresh)return null
  ...
  return {strategyId:'crypto-spot-crossover',strategyVersion:'1',symbol:input.symbol,observations:all,targetPrice:last.mul('1.02').toFixed(2),stopPrice:last.mul('0.99').toFixed(2),validUntil:new Date(new Date(latest.receivedAt).getTime()+15*60_000).toISOString(),riskNotes:['Spot-only informational candidate; deterministic risk validation is required.'],status:'eligible'}
}
