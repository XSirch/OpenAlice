...
import { FixedIncomeClassifier } from './FixedIncomeClassifier'
...
export function OpenFinanceCustody({ onSnapshotChange }: { onSnapshotChange?: (snapshot: CustodySnapshot | null) => void }) {
  ...
  return (
    <section className="border border-border rounded-lg bg-secondary p-5" aria-label="MeuPluggy account settings">
      ...
      {snapshot && <FixedIncomeClassifier snapshot={snapshot} onSaved={() => void refresh()} />}
      {snapshot && <div className="mt-5 border-t border-border pt-4"><div className="mb-3 flex items-center justify-between"><span className="text-[12px] text-muted-foreground">{snapshot.positions.length} positions · refreshed {new Date(snapshot.fetchedAt).toLocaleString()}</span><span className="text-sm font-semibold" title="Sum of net position balances in BRL">{fmt(total, 'BRL')}</span></div><div className="mb-2 grid grid-cols-[minmax(0,1fr)_minmax(9rem,auto)_auto_auto] gap-4 text-[10px] uppercase tracking-wide text-muted-foreground"><span>Asset</span><span>Custodian</span><span>Quantity · unit value</span><span className="min-w-20 text-right">Net balance</span></div><div className="space-y-2">{snapshot.positions.map(position => <div key={position.id} className="grid grid-cols-[minmax(0,1fr)_minmax(9rem,auto)_auto_auto] gap-4 text-[12px]"><span><span className="font-medium text-foreground">{position.code ?? position.name}</span>{position.code && <span className="ml-2 text-muted-foreground">{position.name}</span>}{position.asOf && <span className="block text-[11px] text-muted-foreground">Base date: {new Date(position.asOf).toLocaleDateString()}</span>}</span><span className="text-muted-foreground">{position.institution ?? 'Institution not informed by Pluggy'}</span><span className="text-right text-muted-foreground">{position.quantity == null ? '—' : fmtNum(position.quantity)}{position.unitValue == null ? '' : <span className="block text-[11px]">{fmt(position.unitValue, position.currency)}</span>}</span><span className="min-w-20 text-right text-foreground">{position.value == null ? '—' : fmt(position.value, position.currency)}{position.grossAmount != null && position.grossAmount !== position.value && <span className="block text-[11px] text-muted-foreground">gross {fmt(position.grossAmount, position.currency)}</span>}</span></div>)}</div></div>}
    </section>
  )
}
