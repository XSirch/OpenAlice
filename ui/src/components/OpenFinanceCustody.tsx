...
export function OpenFinanceCustody() {
  ...
  return (
    <section className="border border-border rounded-lg bg-bg-secondary p-5" aria-label="Open Finance custody">
      ...
      {snapshot && <div className="mt-5 border-t border-border pt-4"><div className="mb-3 flex items-center justify-between"><span className="text-[12px] text-text-muted">{snapshot.positions.length} positions · refreshed {new Date(snapshot.fetchedAt).toLocaleString()}</span><span className="text-sm font-semibold" title="Sum of net position balances in BRL">{fmt(total, 'BRL')}</span></div><div className="mb-2 grid grid-cols-[1fr_auto_auto] gap-4 text-[10px] uppercase tracking-wide text-text-muted"><span>Asset</span><span>Quantity · unit value</span><span className="min-w-20 text-right">Net balance</span></div><div className="space-y-2">{snapshot.positions.map(position => <div key={position.id} className="grid grid-cols-[1fr_auto_auto] gap-4 text-[12px]"><span><span className="font-medium text-text">{position.code ?? position.name}</span>{position.code && <span className="ml-2 text-text-muted">{position.name}</span>}<span className="ml-2 text-text-muted">{position.institution ?? ''}</span>{position.asOf && <span className="block text-[11px] text-text-muted">Base date: {new Date(position.asOf).toLocaleDateString()}</span>}</span><span className="text-right text-text-muted">{position.quantity == null ? '—' : fmtNum(position.quantity)}{position.unitValue == null ? '' : <span className="block text-[11px]">{fmt(position.unitValue, position.currency)}</span>}</span><span className="min-w-20 text-right text-text">{position.value == null ? '—' : fmt(position.value, position.currency)}{position.grossAmount != null && position.grossAmount !== position.value && <span className="block text-[11px] text-text-muted">gross {fmt(position.grossAmount, position.currency)}</span>}</span></div>)}</div></div>}
    </section>
  )
}
