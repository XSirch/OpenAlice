...
vi.mock('@/domain/open-finance/pluggy.js', () => ({
  fetchPluggyCustody: vi.fn().mockResolvedValue({
    ...
    positions: [{ id: 'PETR4', name: 'Petrobras', code: 'PETR4', type: 'EQUITY', quantity: 10, value: 411.5, originalAmount: 400, profit: 0, acquiredAt: '2024-01-10T00:00:00.000Z', costBasisSource: 'reported', unitValue: 41.15, currency: 'BRL', institution: 'Banco Demo' }],
  }),
}))
...
describe('PluggyBroker', () => {
  it('normalizes custody into one funded read-only UTA account and positions', async () => {
    ...
    await expect(broker.getPositions()).resolves.toMatchObject([{
      currency: 'BRL', quantity: expect.objectContaining({}), avgCost: '40', marketValue: '411.5', unrealizedPnL: '11.5', acquiredAt: '2024-01-10T00:00:00.000Z', costBasisSource: 'reported', custodian: 'Banco Demo',
